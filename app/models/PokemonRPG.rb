...
class PokemonRPG
    ...
    def count_pokedex
        current_count = UserPokemon.all.uniq.count 
        count_left = 151 - current_count
        puts "You have caught #{current_count} pokemon. You have #{count_left} pokemon left."
    end
    ...
    def battle_prompt
        ...
        battle_options = prompt.select("Choose one:", battle_choices)
            if battle_options == 1
                ...
                if UserPokemon.all.include?(@@random_array.last.id)
                    print TTY::Box.frame pastel.green("You have already caught #{@@random_array.last}.\n#{@@random_array.last.description}. It can be found at the #{@@random_array.last.location}.")
                    battle_prompt
                else 
    ...
    def change_location
        ...
        location_choice = prompt.select("Choose one:", locations)
            if location_choice == 1 
                ...
                return "Thank you for playing Pokemon: Flatiron Edition!" 
            end
    end
    ...
    def run
        play_music("app/models/Pokemontheme.mp3")
        sleep(6)
        title
        sleep(3)
        title_sequence
